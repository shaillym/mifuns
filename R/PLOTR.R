`PLOTR` <-
  function (b, ProjectDir, dvname = NULL, logtrans = FALSE, covplt = FALSE, 
            grp = NULL, grpnames = NULL, cont.cov = NULL, cat.cov = NULL, 
            par.list = NULL, eta.list = NULL, missing = -99, onefile=TRUE) 
{
  start <- getwd()
  ...
  if (!is.null(grp)) {
    ...
    if (length(ignore)) {
      ...
      warning(paste("ignoring grp variable", paste(grp, 
                                                   collapse = ", "), "(not exist in *.TAB file or in data set)"))
    }
  }
  runIDs <- unique(data.table$ID)
  data.file.nC <- data.file[data.file$C != "C" & is.element(data.file$ID, 
                              runIDs), ]
  covariates <- data.file.nC[!duplicated(data.file.nC$ID), 
                             ]
  data.table$ID <- as.character(data.table$ID)
  if (!file.exists(cwtab1)) 
    message(paste("No CWRES plots because file", cwtab1, 
                  "not found in NONMEM run directory."))
  if (file.exists(cwtab1)) {
    if (!"CWRES" %in% names(data.table)) {
      tab.prefix.nm <- paste(ProjectDir, "/", b, "/cwtab", 
                             sep = "")
      cwres.all <- compute.cwres(run.number = 1, tab.prefix = tab.prefix.nm, 
                                 printToOutfile = TRUE)
      data.cwres <- read.table(file = paste(ndir, "/cwtab1", 
                                 sep = ""), skip = 1, header = TRUE)
      data.table$CWRES <- data.cwres$CWRES
      write(paste("Table output from NONMEM for Run ", 
                  b, " with CWRES as last column on", format(Sys.time(), 
                                                             "%a %b %d, %X, %Y"), sep = ""), file = TabFileName, 
            ncolumns = 1)
  ...
  if (!is.null(grp)) {
    ...
    if (length(can)) 
      dataObs <- merge(dataObs, covariates[, c("ID", can)], 
                       by = "ID")
  }
  if (logtrans) {
  ...
  if(!onefile){
      ...
      pdf(paste(ProjectDir, "/", "DiagnosticPlotReview", grp, "_", 
            b, "_%d.pdf", sep = ""), onefile=FALSE, height = 6, width = 6)
    }else
  {
   pdf(paste(ProjectDir, "/", "DiagnosticPlotReview", grp, "_", 
            b, ".pdf", sep = ""), height = 6, width = 6)
 }
  lapply(diagnostics(grp, grpnames, ProjectDir, b, dataObs, 
                     dvname, covplt), print)
  if (!covplt) 
  ...
  if (covplt & !file.exists(TabFileName)) 
    warning(paste("Cov. plots requested but can't find", 
                  TabFileName))
  if (covplt & file.exists(ParFileName) & file.exists(TabFileName)) 
    lapply(doCov(ParFileName, par.list, eta.list, CovFile, 
