		...
		lapply(
			...
			function(strip){
				...
				if(group.number==1) grid.polyline(
					x=rep(z,each=2),
					y=unit(rep(0:1,length.out=length(z)*2),'npc'),
...
Density variant: multistrip panels.
<<boa2,fig=TRUE>>=
print(stripplot(
	quant~value|metric,
	molten,
...
Density variant: interchanging the ordinal and the conditional.
<<boa3,fig=TRUE>>=
print(stripplot(
	metric~value|quant,
	molten,
...
#)   
@
When the bootstraps are complete, we return here and summarize. 
<<more>>=  
boot <- rlog(
	run=1:300,
	project='../nonmem/1005.boot',
...
@
It looks like we have 14 estimated parameters.  We want to map them to the
