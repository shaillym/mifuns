...
function (NMcom, b, ProjectDir, boot = 0, concurrent = TRUE, 
    ...
    par.list = NULL, eta.list = NULL, missing = -99, dosbox = TRUE, nochecksum = FALSE, grid = FALSE) 
{
    start <- getwd()
    Platform <- "Windows"
    ...
    for (i in b) {
        run.args <- list(NMcom = NMcom, ProjectDir = ProjectDir, 
            ...
            par.list = par.list, eta.list = eta.list, missing = missing, dosbox = dosbox, nochecksum = nochecksum, grid = grid)
        if (!concurrent) 
